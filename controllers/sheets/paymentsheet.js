...
const sendPaymentData = async (req, res) => {
    const { Name, Received, ReceivedDate, PaymentMode, Remarks } = req.body;

    if (![Name, Received, ReceivedDate, PaymentMode, Remarks].every(Boolean)) {
    ...
    try {
        await sheets.spreadsheets.values.append({
            ...
            requestBody: { values: [[Name, Received, ReceivedDate, PaymentMode, Remarks]] },
        });
...
const deletePaymentData = async (req, res) => {
    ...
    try {
        const rows = await fetchPaymentsData();
        const index = rows.findIndex(row => row[0] === Name && row[1] == Received && row[2] == ReceivedDate && row[3] == PaymentMode && row[4] == Remarks); // Find index based on email
