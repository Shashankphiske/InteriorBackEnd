...
const sheetId = "1xq89RyuTVGtxYVpuCyQgeYQk9h48EV9nbv-WS32BZzs";
const range = "SalesAssociateData!A:D";

// Utility function to fetch all sales associate data
const fetchPaintsSalesAssociateData = async () => {
    try {
...
const sendPaintsSalesAssociateData = async (req, res) => {
...
const getPaintsSalesAssociateData = async (req, res) => {
    try {
        const rows = await fetchPaintsSalesAssociateData();
        if (!rows.length) {
...
const deletePaintsSalesAssociateData = async (req, res) => {
    const { name } = req.body;
...
const updatePaintsSalesAssociateData = async (req, res) => {
    const { name, email, phonenumber, address } = req.body;
    ...
    try {
        const data = await fetchPaintsSalesAssociateData(); // Fetch all sales associates
        let index = -1;
...
// Export functions
module.exports = { sendPaintsSalesAssociateData, getPaintsSalesAssociateData, deletePaintsSalesAssociateData, fetchPaintsSalesAssociateData, updatePaintsSalesAssociateData };
